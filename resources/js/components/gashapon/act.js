(function ($) {
    var methods = {
        init: function (options) {
			...
			var stack = Composites.stack(sW/2,sH/2 - (shapeAmount * shapeSize), 2,shapeAmount, 0, 0, function(x, y) {
				return Bodies.circle(x, y, shapeSize, {
					...
					render: {
						...
						lineWidth:0,
						sprite: {
							texture: `https://nnemp-product-1254101407.cos.ap-shanghai.myqcloud.com/activities/${Math.floor(Common.random(1,5))}.png`,
							xScale: 1,
							yScale: 1
						}
					}
			...
			var explosion = function(engine) {
				...
				for (var i = 0; i < bodies.length; i++) {
					...
					if (!body.isStatic && body.position.y >= (sW/2.5+r)*0.92) {
